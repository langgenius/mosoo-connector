...
var Specs = []runtime.CommandSpec{
	...
	},
	{
		...
		Notes: []string{"Set top-level maxCostUsd with --set maxCostUsd=<usd-amount> or a numeric JSON field in --file. Choose a positive USD amount with at most six decimal places, within the deployment policy maximum.", "The cap applies only to this turn. Omission uses the deployment's configured default when available; the CLI does not supply a default amount or platform-funded inference.", "A deployment budget policy is required for an explicit cap. Without it the request fails with 409 readiness_blocked. Check the target's v2 schema and budget readiness; the last verified staging source is 69de7cea without budgets, and e060f465 budget deployment and live validation are pending.", "Costs are estimates, not invoices. In-flight usage can exceed the cap; reaching the threshold rejects new model requests, and unknown usage fails closed. Native provider protocols are unchanged.", "Budgeted runs expose run.budget with capUsd, estimatedCostUsd and state: available, settling, budget_exhausted or budget_usage_unavailable. Unavailable usage means the estimate covers only established usage.", "A budget failure remains a failed run. Inspect available files and events; --final-output is only for completed runs, and a failed turn does not guarantee a successful checkpoint."},
		KnownErrors: []runtime.KnownError{
			{Status: 401, Cause: "Invalid or revoked credential. Rotate the Project API key or run mosoo auth login again."},
	...
	},
	{
		...
		Long:    "Create a durable Session from the latest saved private Agent configuration, without publishing. The body and userId are optional; a supplied userId must be a non-blank string. Existing Sessions retain their admitted configuration. Optional top-level maxCostUsd applies to the initial input turn and requires a configured deployment budget policy. In-flight usage can exceed this estimate cap. Per-turn budgets are unreleased; verify support on the target deployment before using maxCostUsd.",
		...
		Examples: []runtime.CommandExample{
			...
			{Summary: "Create an initial turn with a caller-selected estimate cap; set TURN_MAX_COST_USD to your chosen amount first.", Command: "mosoo public-thread-api-v2 threads create --agent-id <agent-id> --set input.type=user.message --set \"input.content[0].type=text\" --set-str \"input.content[0].text=Start this turn.\" --set \"maxCostUsd=$TURN_MAX_COST_USD\" -o json", OutputHints: &runtime.ExampleOutputHints{IDPath: "thread.id"},
			},
		},
		Notes: []string{"Set top-level maxCostUsd with --set maxCostUsd=<usd-amount> or a numeric JSON field in --file. Choose a positive USD amount with at most six decimal places, within the deployment policy maximum.", "The cap applies only to this turn. Omission uses the deployment's configured default when available; the CLI does not supply a default amount or platform-funded inference.", "A deployment budget policy is required for an explicit cap. Without it the request fails with 409 readiness_blocked. Check the target's v2 schema and budget readiness; the last verified staging source is 69de7cea without budgets, and e060f465 budget deployment and live validation are pending.", "Costs are estimates, not invoices. In-flight usage can exceed the cap; reaching the threshold rejects new model requests, and unknown usage fails closed. Native provider protocols are unchanged.", "Budgeted runs expose run.budget with capUsd, estimatedCostUsd and state: available, settling, budget_exhausted or budget_usage_unavailable. Unavailable usage means the estimate covers only established usage.", "A budget failure remains a failed run. Inspect available files and events; --final-output is only for completed runs, and a failed turn does not guarantee a successful checkpoint."},
		KnownErrors: []runtime.KnownError{
